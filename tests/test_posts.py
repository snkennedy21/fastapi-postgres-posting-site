...
def test_get_one_post_not_exist(authorized_client, test_posts):
  res = authorized_client.get(f'/posts/23413523462')
  assert res.status_code == 404

@pytest.mark.parametrize("title, content, published", [
  ("first title", "first content", False),
  ("second title", "second content", True),
  ("third title", "third content", False),
  ("fourth title", "fourth content", True),
])
...
def test_delete_post_non_exist(authorized_client, test_user, test_posts):
  res = authorized_client.delete('/posts/123412341234')
  assert res.status_code == 404
...
def test_update_post_non_exist(authorized_client, test_user, test_posts):
  ...
  res = authorized_client.put('/posts/123412341234', json=data)
  assert res.status_code == 404